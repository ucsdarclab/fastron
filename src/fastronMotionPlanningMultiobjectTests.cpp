...
planResults plan(int d, plannerTypes plannerType, FastronKCD *fastronPtr, Eigen::ArrayXd startVector, Eigen::ArrayXd goalVector)
{
    ...
    case BITSTAR:
        ...
        // std::cout << "USE JIT?: " << space->setLongestValidSegmentFraction(.001) << std::endl;
        break;
    }
    planner->setup();
...
int main(int argc, char **argv)
{
    ...
    primitive_msg.dimensions[0] = 0.1;
    primitive_msg.dimensions[1] = 0.1;
    primitive_msg.dimensions[2] = 0.1;
    ...
    obsLowLim << 0.4, -0.9, 0.2;
    obsUpLim << 0.7, -0.5, 0.6;
    std::string fileLoc = "~/fastronResults/" + std::to_string(d) + "DOF" + std::to_string(static_cast<long int>(std::time(0))) + "colType" + std::to_string(useGJKforFastron) + "numObs" + std::to_string(numObs) + "/";
    ...
    while (ros::ok())
    {
        ...
        // planning_scene.world.collision_objects.front().primitive_poses[0].position.x = 0.55;
        // planning_scene.world.collision_objects.front().primitive_poses[0].position.y = -0.2;
        // planning_scene.world.collision_objects.front().primitive_poses[0].position.z = 0.6;

        // planning_scene.world.collision_objects.front().primitive_poses[1].position.x = 0.55;
        // planning_scene.world.collision_objects.front().primitive_poses[1].position.y = -0.5;
        // planning_scene.world.collision_objects.front().primitive_poses[1].position.z = 0.3;

        // planning_scene.world.collision_objects.front().primitive_poses[2].position.x = 0.65;
        // planning_scene.world.collision_objects.front().primitive_poses[2].position.y = -0.2;
        // planning_scene.world.collision_objects.front().primitive_poses[2].position.z = 0.2;
        ...
        for (int trial = 0; trial < std::atof(argv[5]); ++trial)
        {
            ...
            for (int obs = 0; obs < numObs; ++obs)
            {
                ...
                planning_scene.world.collision_objects.front().primitive_poses[obs].position.x = startPos(0);
                planning_scene.world.collision_objects.front().primitive_poses[obs].position.y = startPos(1);
                planning_scene.world.collision_objects.front().primitive_poses[obs].position.z = startPos(2);
            ...
            while (1)
            {
                ...
                for (int obs = 0; obs < numObs; ++obs)
                {
                    if (planning_scene.world.collision_objects.front().primitive_poses[obs].position.x < obsLowLim(0) ||
                        planning_scene.world.collision_objects.front().primitive_poses[obs].position.x > obsUpLim(0) ||
                        planning_scene.world.collision_objects.front().primitive_poses[obs].position.y < obsLowLim(1) ||
                        planning_scene.world.collision_objects.front().primitive_poses[obs].position.y > obsUpLim(1) ||
                        planning_scene.world.collision_objects.front().primitive_poses[obs].position.z < obsLowLim(2) ||
                        planning_scene.world.collision_objects.front().primitive_poses[obs].position.z > obsUpLim(2))
                    {
                        outOfBounds = true;
                        std::cout << "OOB" << std::endl;
                    }

                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.x += vel(obs, 0);
                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.y += vel(obs, 1);
                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.z += vel(obs, 2);
                }
                if (outOfBounds)
                    break;
                planning_scene_publisher.publish(planning_scene);
                ...
                for (int m = 0; m < 3; ++m)
                {
                    ...
                    {
                        planResults planResult = plan(d, RRTSTAR, &fastron, start, goal);
