...
int collisionDetection(InteractiveRobot &robot, std::vector<std::string> joint_names, Eigen::RowVectorXd query_point, planning_scene::PlanningScene *PlanningScenePtr, collision_detection::AllowedCollisionMatrix *acmPtr, int colDetector = 0)
{
    ...
    if (colDetector)
    {
        ...
        for (int j = 0; j < joint_names.size(); ++j)
        {
            ...
            for (int obs = 0; obs < PlanningScenePtr->getCollisionWorld()->getWorld()->getObjectIds().size(); ++obs)
            {
                fcl::Transform3f tf1(eigen2fcltf(PlanningScenePtr->getCollisionWorld()->getWorld()->getObject("obs" + std::to_string(obs))->shape_poses_.front()));
...
int main(int argc, char **argv)
{
    ...
    moveit_msgs::CollisionObject obs[numObs];
    for (int i = 0; i < numObs; ++i)
    {
        obs[i].id = "obs" + std::to_string(i);
        obs[i].header.frame_id = "/world";
        obs[i].operation = obs[0].ADD;
        obs[i].primitives.push_back(primitive_msg);
    }
    ...
    for (int i = 0; i < numObs; ++i)
        obs[i].primitive_poses.push_back(obs_pose_msg);

    for (int i = 0; i < numObs; ++i)
    {
        moveit_msgs::ObjectColor obsColor;
        obsColor.id = "obs" + std::to_string(i);
    ...
    for (int i = 0; i < numObs; ++i)
        planning_scene.world.collision_objects.push_back(obs[i]);
    ...
    while (ros::ok())
    {
        for (int trial = 0; trial < std::atof(argv[5]); ++trial)
        {
            ...
            for (int obs = 0; obs < numObs; ++obs)
            {
                ...
                planning_scene.world.collision_objects[obs].primitive_poses.front().position.x = startPos(0);
                planning_scene.world.collision_objects[obs].primitive_poses.front().position.y = startPos(1);
                planning_scene.world.collision_objects[obs].primitive_poses.front().position.z = startPos(2);
            ...
            while (1)
            {
                ...
                for (int obs = 0; obs < numObs; ++obs)
                {
                    if (planning_scene.world.collision_objects[obs].primitive_poses.front().position.x < obsLowLim(0) ||
                        planning_scene.world.collision_objects[obs].primitive_poses.front().position.x > obsUpLim(0) ||
                        planning_scene.world.collision_objects[obs].primitive_poses.front().position.y < obsLowLim(1) ||
                        planning_scene.world.collision_objects[obs].primitive_poses.front().position.y > obsUpLim(1) ||
                        planning_scene.world.collision_objects[obs].primitive_poses.front().position.z < obsLowLim(2) ||
                        planning_scene.world.collision_objects[obs].primitive_poses.front().position.z > obsUpLim(2))
                    {
                        outOfBounds = true;
                        std::cout << "OOB" << std::endl;
                    }

                    planning_scene.world.collision_objects[obs].primitive_poses.front().position.x += vel(obs, 0);
                    planning_scene.world.collision_objects[obs].primitive_poses.front().position.y += vel(obs, 1);
                    planning_scene.world.collision_objects[obs].primitive_poses.front().position.z += vel(obs, 2);
                }
                if (outOfBounds)
                    break;
