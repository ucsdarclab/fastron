...
int collisionDetection(InteractiveRobot &robot, std::vector<std::string> joint_names, Eigen::RowVectorXd query_point, planning_scene::PlanningScene *PlanningScenePtr, collision_detection::AllowedCollisionMatrix *acmPtr, int colDetector = 0)
{
    ...
    else
    {
        ...
        c_req.cost = false;
        // std::cout << c_req.max_contacts << std::endl;
        ...
        // PlanningScenePtr->checkCollision(c_req, c_res, *robot.robotState());
    ...
    while (ros::ok())
    {
        for (int trial = 0; trial < std::atof(argv[5]); ++trial)
        {
            ...
            for (int obs = 0; obs < numObs; ++obs)
            {
                ...
                // Eigen::ArrayXd startPos(3);
                // if (obs == 0)
                //     startPos << 0.5, -0.0+1, 0.3;
                // else if (obs == 1)
                //     startPos << 0.5, -0.4+1, 0.3;
                // else if (obs == 2)
                //     startPos << 0.5, -0.8+1, 0.3;
            ...
            while (1)
            {
                ...
                for (int obs = 0; obs < numObs; ++obs)
                {
                    ...
                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.x += 0;//vel(obs, 0);
                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.y += -0.01;//vel(obs, 1);
                    planning_scene.world.collision_objects.front().primitive_poses[obs].position.z += 0.0;//vel(obs, 2);
                }
                if (outOfBounds)
                    break;
                planning_scene_publisher.publish(planning_scene);
                ...
                Eigen::RowVectorXd q = Eigen::RowVectorXd::Zero(d);
                // std::cout << ((fastron.kcd(q) > 0) ? "Collision detected" : "") << std::endl ;
                Eigen::ArrayXd y(100);
                clock_t t_mu;
                t_mu = clock();
                for (int i = 0 ; i < 100 ; ++i)
                ...
                std::cout << (clock() - (double)t_mu) / CLOCKS_PER_SEC * 1e6/100 << " " << y.sum() << std::endl;
                ...
                //     {
                //         planResults planResult = plan(d, RRT, &fastron, start, goal);
