...
planResults plan(int d, plannerTypes plannerType, FastronKCD *fastronPtr, Eigen::ArrayXd startVector, Eigen::ArrayXd goalVector)
{
    ...
    if (solved)
    {
        ...
        if (!planResult.isValid)
        {
            ...
            Eigen::MatrixXd segment1, segment3;
            segment1 = planResult.path.topRows(startIdx);
            segment3 = planResult.path.bottomRows(planResult.path.rows() - goalIdx - 1);

            planResult.path.resize(segment1.rows() + planResultRepair.path.rows() + segment3.rows(), Eigen::NoChange);
            planResult.path << segment1, planResultRepair.path, segment3;

            // planResult.path = planResultRepair.path.rows();
...
int main(int argc, char **argv)
{
    ...
    Eigen::MatrixXd data = Eigen::MatrixXd::Random(N + 2, d);
    ...
    while (ros::ok())
    {
        ...
        for (int trial = 0; trial < std::atof(argv[5]); ++trial)
        {
            ...
            Eigen::ArrayXd start(d), goal(d);
            start << 1.5, -0.4, 0, 1.3, 0, -0.5, 0;
            goal << -1, -0.4, 0, 1.3, 0, -0.5, 0;
            data.row(data.rows() - 2) = start.matrix();
            data.row(data.rows() - 1) = goal.matrix();
            ...
            fastron.updateModel();
            fastron.validityMethod = 0; // FCL: 0, GJK: 1, Fastron: 2
            ...
            // upper lim: 1.70168   1.047 3.05418   2.618   3.059   2.094   3.059
            // lower lim: -1.70168   -2.147 -3.05418    -0.05   -3.059  -1.5708   -3.059
            // for (int i = 0; i < r_joint_names.size(); ++i)
            // {
            //     robot.robotState()->setJointPositions(r_joint_names[i], start.data() + i);
            // }
            // moveit::core::robotStateToRobotStateMsg(*robot.robotState(), displayRobotState.state);
            // r_arm_pub.publish(displayRobotState);
            start = fastron.unscaleConfiguration(start);
            goal = fastron.unscaleConfiguration(goal);

            fastron.validityMethod = 2; // FCL: 0, GJK: 1, Fastron: 2
            planResults planResult;
            // if (reverse)
            //     planResult = plan(d, RRTCONNECT, &fastron, goal, start);
            // else
            planResult = plan(d, RRT, &fastron, start, goal);

            // std::cout << planResult.path << std::endl;
            Eigen::RowVectorXd pathTemp;
            for (int j = 0; j < planResult.path.rows(); ++j)
            {
                pathTemp = planResult.path.row(j);
                pathTemp = fastron.scaleConfiguration(pathTemp);
                // std::cout << j << ":" << pathTemp << std::endl;
                for (int i = 0; i < r_joint_names.size(); ++i)
                {
                    robot.robotState()->setJointPositions(r_joint_names[i], pathTemp.data() + i);
                    // robot.robotState()->setToRandomPositions();
                    // robot.robotState()->setJointPositions(l_joint_names[i], start_config_display_left.data() + i);
                }
                moveit::core::robotStateToRobotStateMsg(*robot.robotState(), displayRobotState.state);
                r_arm_pub.publish(displayRobotState);
            ...
            // if (planResult.path.rows() > 1)
            // {
            //     if (reverse)
            //     {
            //         std::cout << (pathTemp.transpose().array() - start.array()).matrix().norm() << std::endl;
            //     }
            //     reverse = !reverse;
            // }
